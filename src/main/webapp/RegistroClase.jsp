                    ...
                    <h1 class="font-weight-bold mb-3"> Registro de Socio a una Clase</h1>
                      ...
                      <form action="RegistroClase" method="post">
							...
							%>         
								...
								<%
									String horaI;
									if(Dc.getHoraInicio().getMinutes()<=9) {
										horaI=String.valueOf(Dc.getHoraInicio().getHours() + ":0" +Dc.getHoraInicio().getMinutes());
									}
									else {
										horaI=String.valueOf(Dc.getHoraInicio().getHours() + ":" +Dc.getHoraInicio().getMinutes());
									}
								%>
								<%
									String horaR;
									if(Dc.getFechaReg().getMinutes()<=9) {
										horaR=String.valueOf(Dc.getFechaReg().getHours() + ":0" +Dc.getFechaReg().getMinutes());
									}
									else {
										horaR=String.valueOf(Dc.getFechaReg().getHours() + ":" +Dc.getFechaReg().getMinutes());
									}
								%>					
								align="right" width="400" height="400">Nombre: <%=Dc.getNombre()%>&nbsp &nbsp &nbsp &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
								<br>Fecha: <%=Dc.getFecha().getDate()%>/<%=(Dc.getFecha().getMonth()+1)%>/<%=(Dc.getFecha().getYear()+1900)%>       
								<br>Hora de Inicio: <%=horaI%>
								<br>Url: <%=Dc.getUrl()%>
								<br>Fecha de Reguistro:<%=Dc.getFechaReg().getDate()%>/<%=(Dc.getFechaReg().getMonth()+1)%>/<%=(Dc.getFechaReg().getYear()+1900)%><%=" "+horaR%> <br>
								</p>
				                </FONT>
